...
# subset to only one type of cells, stromal is the most abundant, but lets focus on invasive tumour
...
knn_out = queryKNN(reducedDim(spe_raw[ , spe_raw$Cluster != ct & spe_raw$Cluster != "Unlabeled" ], "spatial"),
                   reducedDim(spe, "spatial"),
                   k = 1)
knn_tab = unclass(table(droplevels(spe_raw[ , spe_raw$Cluster != ct & spe_raw$Cluster != "Unlabeled" ]$Cluster[knn_out$index[,1]]), spe$m))
...
sm_all = scoreMarkers(spe_raw, groups = spe_raw$Cluster)[[ct]]
...
plotReducedDim(spe, "spatial", colour_by = "Annotation") + theme(aspect.ratio = 1)
...
ggsave(g, file = paste0(fig_dir, "visium_wSIR_UMAP_annotation.pdf"), height = 4, width = 4)
ggsave(g, file = paste0(fig_dir, "visium_wSIR_UMAP_annotation.png"), height = 4, width = 4)
...
g = plotReducedDim(spe, "spatial", colour_by = "Annotation",point_shape = 16, point_alpha = 1, point_size = 2) + 
  ...
  theme(legend.position = "none") + 
  ggtitle("Visium - Annotation")

g

ggsave(g, file = paste0(fig_dir, "visium_wSIR_spatial_annotation.pdf"), height = 5, width = 5)
ggsave(g, file = paste0(fig_dir, "visium_wSIR_spatial_annotation.png"), height = 5, width = 5)
...
ggsave(g_leg, file = paste0(fig_dir, "visium_annotation_legend.pdf"), height = 4, width = 2.3)
...
ggsave(g, file = paste0(fig_dir, "Visium_wSIR_PCA_clusters_confusion.pdf"), height = 5, width = 5.5)
ggsave(g, file = paste0(fig_dir, "Visium_wSIR_PCA_clusters_confusion.png"), height = 5, width = 5.5)
...
g = wrap_plots(sapply(c("MMP12", "OPRPN", "POSTN", "PTGDS", "SFRP4"), function(gene) {
  plotReducedDim(spe, "spatial", colour_by = gene, point_shape = 16, point_alpha = 1, point_size = 0.45) + 
    guides(colour = guide_legend(override.aes = list(size = 10), title = "")) + 
    ggtitle(gene) +
    theme(axis.text = element_blank()) +
    theme(axis.line = element_blank()) +
...
ggsave(g, file = paste0(fig_dir, "visium_important_spatial.pdf"), height = 6, width = 10)
ggsave(g, file = paste0(fig_dir, "visium_important_spatial.png"), height = 6, width = 10)
